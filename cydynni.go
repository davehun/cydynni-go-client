...
email := ""
password := ""
u, _ := url.ParseRequestURI(apiURL + "/" + login + "email=" + url.QueryEscape(email) + "&password=" + url.QueryEscape(password))
...
home, err := timePowerMap(getString(homeHalfHourData))
if err != nil {
panic(err)
}
fmt.Println(home)

hydroHalf, err := timePowerMap(getString(hydroHalfHour))
if err != nil {
panic(err)
}
fmt.Println(hydroHalf)

communityHalf, err := timePowerMap(getString(comunityHalfHourData))
if err != nil {
panic(err)
}
